...
export function resolvePort(portValue?: string | number): number {
  if (typeof portValue === 'number') {
    if (portValue > 0 && portValue <= 65535) return portValue;
    return 3000;
  }
...
export async function startServer(portOverride?: number) {
  ...
  // Client static assets & Vite dev server middleware
