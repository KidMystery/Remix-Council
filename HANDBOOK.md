...
- **Nexus "☁️ Run on server"** — Night Shift can execute as a server agent job: launch, close the tab, and the mission keeps falsifying; on return (or reload) the results hydrate into the mission view (passes → consensus rounds, final verdict with sources, Morning Brief, real USD spent). Pause cancels the job; Reset/Follow-up clean it up. The persisted mission carries the `serverJobId` for re-attach.
