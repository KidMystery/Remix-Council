...
| **List fields** | Drive v3 `files.list` on `appDataFolder` must be `fields=files(id,name)` **never** `etag`. v3 `File` has no `etag` field in list; the string `files(id,name,etag)` 400s. ETag for `If-Match` comes from the **GET media header**, not the list body. Helper `driveAppDataListUrl()` enforces this. |
