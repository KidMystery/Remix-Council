...
- **Model recommendations by task** — the Chamber detects the task domain (code / math / finance / creative / general) and, when "Auto-select models" is on (Settings), **overrides models for that run only**. Settings still shows whatever you last picked — that is why it feels decorative. Domain shows in the summary bar ("Domain: CODE"). Live-catalog substitution still runs. Default preset is **Highest Quality** (frontier team). Cheap/Auto-low is an explicit budget pick. Turn auto-select **off** if you want the seats you can see. Handoff does not change this.
