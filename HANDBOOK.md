...
- **Model recommendations by task** — the Chamber detects the task domain (code / math / finance / creative / general) and, when "Auto-select models" is on (Settings), **overrides models for that run only**. Settings still shows whatever you last picked — that is why it feels decorative. Domain shows in the summary bar ("Domain: CODE"). Live-catalog substitution still runs. Default preset is **Balanced Quality** (`cheap` tier), so auto-select will often seat Flash / 4o-mini even if you parked Pro models in Settings. Turn auto-select **off** if you want the seats you can see. Handoff does not change this.
...
- **Per-round cost ceiling (client + server)** — Settings → Advanced → "Per-Round Cost Ceiling". **0 = unlimited** on client and server (no hidden $2 default). When set, the Chamber skips remaining stages and the server refuses extra calls (HTTP 409) using real usage. Hitting the ceiling **blocks the docket** — it is not a stamped verdict. Missing usage under a ceiling is `cost_unknown`.
...
- **Nexus "⚡ Agent Mode" (default execution mode)** — Nexus now leads with the agent-mode experience: you give it a question (finances, a medical bill, a diagnosis, a construction plan — anything), and the server-side agent loop **assesses it, drafts a visible plan (summary + steps + research queries), researches with live citations, deliberates with self-falsification passes, fact-checks the verdict against the citations, then answers** with sources and honest confidence. The mission panel shows the plan, each research pass with clickable source chips, live phase/usage, and a completed "Agent Mission Report". It runs server-side — close the tab and it keeps working; results hydrate on return (passes → consensus cards, final verdict, Morning Brief when Night Shift is on). The classic modes (Autonomous / Mini Delib / Rotation) remain for the multi-persona local loop, and "☁️ Run on server" upgrades those too. Live Research toggle maps to research passes (off = knowledge-only reasoning, zero tool fees).
...
- **Oracle → Chamber handoff** — type `/chamber` (or tap **Send to Chamber**). Oracle does **not** copy the thread. It writes a one-page **Case brief** (question + last few turns + Thread/Global Bible excerpts) and opens a new Chamber session. You still press Deliberate. After a **stamped** verdict, **Admit to Bible** writes only the invariants — never the essay, never an unstamped draft. Tests: `src/lib/__tests__/chamberHandoff.test.ts`.
...
| **Union** | Merge is by id. Chamber rounds pick exhibit identity + completed seats (`preferIncomingRound`), never synthesis length. Oracle messages union by id so a phone turn and a laptop turn both survive. |
...
- **Oracle threads + Bibles** → `council-oracle-threads-v1` and `council-oracle-global-bible-v1` (localStorage), JSON export/import (Oracle header), **and** Google Drive `appDataFolder/council-oracle.json` when signed in (same merge-before-put + tombstones).
