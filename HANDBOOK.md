...
- **Oracle model & modes (Settings, not the main page)** — the Oracle main page stays clean; which model(s) it uses and how is configured in **Settings → Oracle → Model & Modes**: mode picker (Direct / Mini Deliberation / Auto-Rotate), a single model picker for Direct, and roster chip editors (add/remove/randomize/defaults) for the other two. The curated list is current-frontier only.
...
3. **Server-side cost governor** — today the Chamber enforces ceilings client-side. A hard server-side ceiling (using the real `usage` the server already receives) is the remaining money-safety item.
4. **Blind Pro Compare** — the old Phase-2 toggle was a no-op and has been removed. Rebuilding it (an independent adversarial critic model that benchmarks the council's synthesis, shown as a card) is a candidate feature — flag it if you want it.
