...
- **Oracle custom models (Settings → Oracle)** — type a name (`glm`, `claude`, `muse spark`) and the live catalog completes it. You do not need the exact `provider/slug`. The pick is validated against the catalog and badged **Live / Delisted / Unverified (offline)** with **Vision / Text-only**, then joins the Direct palette and every roster picker. Provider errors retry via **OpenRouter Auto** (`openrouter/auto`) — the reply chip shows which model actually answered. No hardcoded Gemini “just use Flash” button. Vision turns pick a **live catalog vision model**, or Auto if the catalog is empty. Delisted ids are always shown, never silently dropped. New Oracle threads **inherit** the active thread's mode, rosters, and toggles.
